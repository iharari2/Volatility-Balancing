...
async function fetchLatestTimeline(portfolioId: string, positionId: string): Promise<TimelineRow[]> {
  const url = `/v1/tenants/${TENANT_ID}/portfolios/${portfolioId}/positions/${positionId}/timeline?limit=10`;
...
export default function DecisionMonitor({
  ...
  const deltaVsAnchor = latest.anchor_delta_pct ??
    (latest.anchor_price_at_eval && latest.effective_price
      ? ((latest.effective_price - latest.anchor_price_at_eval) / latest.anchor_price_at_eval) * 100
      : null);
    ...
    <div className="bg-white border border-gray-200 rounded-lg">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 divide-x divide-y lg:divide-y-0 divide-gray-200">
        ...
        <div className="p-4">
          ...
          <div className="text-xs text-gray-500 mt-1">
            Anchor: ${(latest.anchor_price_at_eval || 0).toFixed(2)}
          </div>
        </div>
        ...
        <div className="p-4">
          ...
          <div className={`text-xl font-bold flex items-center gap-2 ${
            latest.trigger_detected ? 'text-orange-600' : 'text-gray-400'
          }`}>
            {triggerIcon}
            {latest.trigger_detected
        ...
        <div className="p-4">
          ...
          <div className="text-xs text-gray-500 mt-1 truncate" title={latest.guardrail_block_reason || ''}>
            {latest.guardrail_block_reason || `Stock: ${(latest.position_stock_pct_before || 0).toFixed(1)}%`}
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-gray-200">
        ...
        <div className="divide-y divide-gray-100 max-h-48 overflow-y-auto">
          {timeline.slice(0, 5).map((row, idx) => {
            const rowDelta = row.anchor_delta_pct ??
              (row.anchor_price_at_eval && row.effective_price
                ? ((row.effective_price - row.anchor_price_at_eval) / row.anchor_price_at_eval) * 100
                : null);
