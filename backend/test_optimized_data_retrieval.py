...
print('Testing optimized data retrieval (30 days):')
...
try:
    ...
    if response.status_code == 200:
        ...
        expected_total = expected_per_day * 30  # 30 days
        print(f'Expected evaluations: {expected_total} ({expected_per_day} per day × 30 days)')
        print(f'Actual evaluations: {actual_total}')
        print(f'Ratio: {actual_total/expected_total:.2f}')
        
        # Check for weekend/holiday filtering
        if actual_total/expected_total > 0.8:
            print('✅ Good data retrieval - properly filtering weekends/holidays')
        else:
            print('❌ Poor data retrieval - may still include non-trading days')
    else:
