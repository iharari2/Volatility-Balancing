...
class TestDividendAPI:
    ...
    def test_get_dividend_status_success(self, client, position_id):
        """Test getting dividend status for existing position."""
        response = client.get(f"/v1/dividends/positions/{position_id}/status")
    ...
    def test_process_ex_dividend_no_dividend_today(self, client, position_id):
        """Test processing ex-dividend when no dividend today."""
        response = client.post(f"/v1/dividends/positions/{position_id}/process-ex-dividend")
    ...
    def test_dividend_workflow_integration(self, client, position_id):
        """Test complete dividend workflow integration."""
        # 1. Set anchor price
        response = client.post(
            f"/v1/positions/{position_id}/anchor",
            params={"price": 150.0}
        )
        assert response.status_code == 200
        ...
        # 4. Get position details to verify anchor price
        response = client.get(f"/v1/positions/{position_id}")
        assert response.status_code == 200
        position = response.json()
        assert position["anchor_price"] == 150.0

    def test_dividend_status_with_receivables(self, client, position_id):
    ...
    def test_api_error_handling(self, client):
        ...
        response = client.post("/v1/dividends/announce", json={
            "ticker": "AAPL"
    ...
    def test_dividend_endpoints_consistency(self, client, position_id):
        """Test that dividend endpoints return consistent data."""
        # Get initial status
        response1 = client.get(f"/v1/dividends/positions/{position_id}/status")
        assert response1.status_code == 200
        status1 = response1.json()
        
        # Get status again
        response2 = client.get(f"/v1/dividends/positions/{position_id}/status")
